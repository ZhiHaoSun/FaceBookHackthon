...
def app():
    ...
    env.project = 'crptcmexchange'
    #  hg repository of your project
    env.repository = 'https://bitbucket.org/chrishan/crptcmexchange'
    #  hosts to deploy your project, users must be sudoers
    env.hosts = ['155.69.148.66', ]
    env.user = "lily"
    env.password = "friend12345"
    # env.hosts = ['155.69.146.186', ]
    # env.user = "root"
    ...
    env.package_root = join(env.code_root, 'crptcmexchange')

    #  the Python path to a Django settings module.
    env.django_project_settings = 'crptcmexchange.settings'
    #  django media dir
    env.django_media_path = join(env.package_root, 'site_media', 'media')
    #  django static dir
    env.django_static_path = join(env.package_root, 'site_media', 'static')
    #  django media url and root dir
    env.django_media_url = '/site_media/media/'
    env.django_media_root = env.package_root
    #  django static url and root dir
    env.django_static_url = '/site_media/static/'
    env.django_static_root = env.package_root
    ...
    env.requirements_file = join(env.code_root, 'production_requirements.txt')
    env.local_settings_file = join(env.package_root, 'local_settings.py')
    ...
    env.gunicorn_bind = "127.0.0.1:8102"
    ...
    # env.nginx_server_name = '155.69.146.186'  # Only domain name, without 'www' or 'http://'
    env.nginx_server_name = 'tcmexchange.ntulily.org'
