...
def _upload_local_settings():
    if isfile('conf/local_settings.py'):
        template = 'conf/local_settings.py'
    else:
        template = '%s/conf/local_settings.py' % fagungis_path
    upload_template(template, env.local_settings_file,
                    context=env, backup=False, use_sudo=True)
