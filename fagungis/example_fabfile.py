...
def myapp():
    ...
    env.repository = 'https://bitbucket.org/myname/myapp'
    #  hosts to deploy your project, users must be sudoers
    env.hosts = ['ec2.us-west-2.compute.amazonaws.com', ]
    env.user = "ubuntu"
    ...
    env.package_root = join(env.code_root, 'myapp')

    #  the Python path to a Django settings module.
    env.django_project_settings = 'myapp.settings'
    #  django media dir
    env.django_media_path = join(env.package_root, 'media')
    #  django static dir
    env.django_static_path = join(env.package_root, 'static')
